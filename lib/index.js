...
function isFullPage() {
  return 'FULL_PAGE' in process.env
    || (process.argv.slice(2).indexOf('--full-page') !== -1);
}
function getTakeSnapshotPos() {
  return process.argv.slice(2).indexOf('--take-snapshot');
}
function isTakeSnapshot() {
  return 'TAKE_SNAPSHOT' in process.env || getTakeSnapshotPos() > -1;
}
function snapshotName() {
  return process.env.SNAPSHOT_NAME
    || (getTakeSnapshotPos() > -1 ? process.argv.slice(2)[getTakeSnapshotPos() + 1] : null);
}

function getType() {
  return (snapshotName() && snapshotName().match(/^[a-z\d_]/) ? snapshotName() : null)
    || (getTakeSnapshotPos() > -1 ? 'base' : 'actual');
}
...
function _takeSnapshot(t, opts, extra) {
  ...
  const imagePath = `${normalize(groupId)}/${filename}/${options.as || getType()}.png`;
    ...
    .then(() => {
      ...
      if (options.blockOut) {
        ...
        return Promise.all(blocks.map(x => {
          ...
          const blockOutName = `blockOut-${getType()}.json`;
          const metaFile = path.join(baseDir, blockOutName);

          fs.writeFileSync(metaFile, JSON.stringify(results.filter(Boolean).filter(a => a.width > 0 && a.height > 0)));
        });
      }
    });
}
